...
def list_projects(request, org_id):
    ...
    relevant_admin = user_roles.filter(name__in=[project_admin_str, org_admin_str]).exists()
    logger.debug(f">>> === {user_roles}, relevant_admin: {relevant_admin} === <<<")
    ...
    if search_query:
        ...
        tobjects = Project.objects.filter(active=True, org_id=org_id, **viewable_dict).order_by('position')
